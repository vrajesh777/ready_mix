			...
			<div class="card-header">
				<form id="filterForm">
					<div class="row align-items-top">
						<div class="col-md-3">
							<label><?php echo e(trans('admin.date')); ?></label>
							<div class="position-relative p-0">
								<input class="form-control pr-5 mainDateInput" name="date" placeholder="<?php echo e(trans('admin.date')); ?>" value="<?php echo e($date??''); ?>">
								<div class="input-group-append date-icon"><i class="fal fa-calendar-alt"></i></div>
								<input class="mr-2 mt-2" type="checkbox" value="1" id="show_all_del_note" name="show_all" <?php if(isset($show_all) && $show_all!='' && $show_all == '1'): ?> checked <?php endif; ?>>Show all delivery
		        			</div>
						</div>

						<div class="col-md-2 offset-2">
							<div class="position-relative p-0">
								<label><?php echo e(trans('admin.time')); ?></label>
								<input class="form-control pr-5 commonTime" disabled="">
		        			</div>
						</div>
						<div class="col-md-2">
							<div class="position-relative p-0">
								<label><?php echo e(trans('admin.sum_of_total_order')); ?></label>
								<input class="form-control pr-5" disabled="" value="<?php echo e($total_order_qty); ?>">
		        			</div>
						</div>
						<div class="col-md-3 ">
							<label><?php echo e(trans('admin.total_cbm')); ?></label>
							<input class="form-control pr-5" value="<?php echo e($tot_cbm ?? 00); ?>" disabled="">
						</div>
				    </div>
			    </form>
			</div>
					...
					<table class="table table-striped table-nowrap custom-table mb-0" id="leadsTable">
						<thead>
							<tr>
								...
								<th class="all text-right notexport"><?php echo e(trans('admin.actions')); ?></th>
								<th class="none"></th>
							</tr>
						</thead>
						<tbody>
						...
						<?php if(last(request()->segments()) !== 'all_delivered'? $remain_qty : !$remain_qty){?>
							<tr>
	                            ...
	                            <td class="text-center">
	                            	<?php if($row['edit_quantity']=='' && ($tot_delivered_quant??0) == ($row['quantity']??0)): ?>
	                            		<button class="btn btn-success btn-sm" type="button"><i class="fa fa-check" title="<?php echo e(trans('admin.delivered')); ?>"></i></button>
	                            	<?php elseif($row['edit_quantity']!='' && ($tot_delivered_quant??0) == ($row['edit_quantity']??0)): ?>
	                            		<button class="btn btn-success btn-sm" type="button"><i class="fa fa-check" title="<?php echo e(trans('admin.delivered')); ?>"></i></button>
	                            	<?php else: ?>
	                            		<?php if($obj_user->hasPermissionTo('dispatch-order-update')): ?>
											<?php if(((date('Y-m-d') <= $order['extended_date']) && (date('Y-m-d') >= $order['extended_date'])) && ((date('Y-m-d') <= $date) && date('Y-m-d') >= $date)): ?>
												<button type="button" class="btn btn-primary btn-sm btn-del-note" data-ord-det_id="<?php echo e(base64_encode($row['id'])); ?>"><i class="fa fa-truck" aria-hidden="true" title="<?php echo e(trans('admin.delivery_note')); ?>"></i></button>
											<?php endif; ?>
										<?php endif; ?>

										<?php if($obj_user->hasPermissionTo('dispatch-order-update')): ?>
											<?php if(date('Y-m-d') <= $order['extended_date']): ?>
												<a class="btn btn-primary btn-sm btn-edit-qty pay_mod" href="javascript:void(0);" id="pay_mod" data-toggle="modal" data-target="#edit_qty_model" data-ord-det-id="<?php echo e(base64_encode($row['id'])); ?>" <?php if(isset($row['edit_quantity']) && $row['edit_quantity']!=''): ?> data-ord-qty="<?php echo e($row['edit_quantity'] ?? 0); ?>" <?php else: ?> data-ord-qty="<?php echo e($row['quantity'] ?? 0); ?>" <?php endif; ?> data-dlv-qty="<?php echo e($tot_delivered_quant); ?>"><i class="fa fa-edit" title="<?php echo e(trans('admin.edit_qty')); ?>" aria-hidden="true"></i></a>
											<?php endif; ?>
										<?php endif; ?>
									<?php endif; ?>
								</td>
								<td>
									<?php if(isset($del_notes) && !empty($del_notes)): ?>
						            <table>
						            	<thead>
									        <tr>
									    ...
									    <tbody>
									    	<?php $__currentLoopData = $del_notes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $note): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
										    	<?php
										    		$driver = $note['driver']??[];
										    	...
										    	<?php if($del_date == $req_date): ?>
											    	<tr>
											            ...
											            <td>
											            	...
											            	<?php if($obj_user->hasPermissionTo('dispatch-order-update') && $note['reject_by'] == ''): ?>
											            		<?php if($del_date == date('Y-m-d')): ?>
											            		<a class="btn btn-primary btn-sm rej_mod" href="javascript:void(0);" data-toggle="modal" data-target="#reject_qty_model" data-note-id="<?php echo e(base64_encode($note['id'])); ?>" data-qty="<?php echo e($note['quantity']??''); ?>" ><i class="fa fa-ban" title="<?php echo e(trans('admin.reject_excess_quantity')); ?>"></i></a>
											            		<?php endif; ?>
											            	<?php endif; ?>
											            	<?php if($note['reject_by']!=''): ?>
											            		<a class="btn btn-primary btn-sm rej_details" id="rej_details" href="javascript:void(0);" data-toggle="modal" data-target="#details_model" data-rejected-by="<?php echo e($note['reject_by'] ?? ''); ?>" data-rejected-reason="<?php echo e($note['remark']?? 0); ?>" data-rejected-qty="<?php echo e($note['reject_qty']?? 0); ?>" data-excess-qty="<?php echo e($note['excess_qty']?? 0); ?>" data-note-id="<?php echo e(base64_encode($note['id'])); ?>"><i class="fa fa-eye" title="<?php echo e(trans('admin.details')); ?>"></i></a>
											            	<?php endif; ?>
											            	<?php if($obj_user->hasPermissionTo('dispatch-order-update') && $note['status']!='cancelled'): ?>
											            		<?php if($del_date == date('Y-m-d')): ?>
											            		<a class="btn btn-primary btn-sm" href="<?php echo e(Route('cancel_note',base64_encode($note['id']))); ?>" onclick="confirm_action(this,event,'Do you really want to cancel this note ?');"><i class="fa fa-window-close" title="Cancel"></i></a>
											            		<?php endif; ?>
											            	<?php endif; ?>
											            </td>
											    	</tr>
										    	<?php elseif($show_all!=0 && $show_all == '1'): ?>
											    	<tr>
											    		<td><?php echo e(($key+1)); ?></td>
											            <td><?php echo e($vehicle['name']??''); ?>&nbsp;
											            	(<?php echo e($vehicle['plate_no']??''); ?> <?php echo e($vehicle['plate_letter']??''); ?>)
											            ...
											            <td>
											           		<?php if($note['reject_by']!='' && $note['reject_by'] == '1'): ?>
											           			<?php echo e(trans('admin.readymix')); ?>
	...
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h3 class="modal-title"><i class="fal fa-receipt text-black-50 mr-1"></i><?php echo e(trans('admin.delivery_note')); ?></h3>
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary del_note_form_submit"><?php echo e(trans('admin.save')); ?></button>
			</div>
		</div>
	</div>
</div>
...
<div class="modal fade right" id="edit_qty_model" tabindex="-1" role="dialog" aria-modal="true">
	<div class="modal-dialog" role="document">
		...
		<div class="modal-content">
			...
			<div class="modal-header">
                <h4 class="modal-title text-center"><?php echo e(trans('admin.edit_quantity')); ?></h4>
                <button type="button" class="close xs-close" data-dismiss="modal">×</button>
            </div>

			<div class="modal-body">

				<form method="POST" action="<?php echo e(Route('edit_del_qty')); ?>" id="update_qty">
					<?php echo e(csrf_field()); ?>

					<input type="hidden" name="enc_order_id" id="enc_order_id">
					<div class="row">
						<div class="form-group col-sm-6">
							<label><?php echo e(trans('admin.quantity')); ?> (M³)</label>
							<input type="text" class="form-control quantity" name="quantity" placeholder="<?php echo e(trans('admin.quantity')); ?> (M³)" data-rule-required="true" data-rule-number="true">
...
<div class="modal fade right" id="reject_qty_model" tabindex="-1" role="dialog" aria-modal="true">
	<div class="modal-dialog" role="document">
		...
		<div class="modal-content">
			...
			<div class="modal-header">
                <h4 class="modal-title text-center"><?php echo e(trans('admin.reject_excess_quantity')); ?></h4>
                <button type="button" class="close xs-close" data-dismiss="modal">×</button>
            </div>

			<div class="modal-body">

				<form method="POST" action="<?php echo e(Route('reject_del_qty')); ?>" id="frmRejectQty">
					<?php echo e(csrf_field()); ?>
					...
					<div class="row">
						<div class="form-group col-sm-6">
							<label><?php echo e(trans('admin.quantity')); ?> (M³)</label>
							<input type="text" class="form-control reject_qty" name="reject_qty" placeholder="<?php echo e(trans('admin.quantity')); ?> (M³)" data-rule-required="true" data-rule-number="true">
							<label class="error" id="reject_qty_error"></label>
						</div>
						<div class="form-group col-sm-6">
							<label><?php echo e(trans('admin.rejected_excess_by')); ?></label>
							<select name="reject_by" class="select2" id="reject_by" data-rule-required="true">
								...
								<option value="1"><?php echo e(trans('admin.internal_reason')); ?></option>
								<option value="2"><?php echo e(trans('admin.customer_end')); ?></option>
								<option value="3"><?php echo e(trans('admin.excess_qty')); ?></option>
							</select>
							<label id="reject_by-error" class="error" for="reject_by"></label>
						</div>
						<div class="form-group col-sm-12">
							<label><?php echo e(trans('admin.remark')); ?></label>
							<textarea name="remark" rows="2" cols="2" class="form-control" placeholder="<?php echo e(trans('admin.remark')); ?>" ></textarea>
						</div>
						...
						<div class="form-group col-sm-6 is_transfer" style="display:none">
							...
							<select name="is_transfer" class="select2" id="is_transfer">
								<option value=""><?php echo e(trans('admin.select')); ?></option>
								<option value="1"><?php echo e(trans('admin.transfer')); ?></option>
								<option value="2"><?php echo e(trans('admin.lost_wastage')); ?></option>
							</select>
							<label id="is_transfer-error" class="error" for="is_transfer"></label>
						</div>

						<div class="form-group col-sm-6 transfer_to" style="display:none">
							<label><?php echo e(trans('admin.transfer_to')); ?></label>
							<select name="transfer_to" class="select2" id="transfer_to">
								<option value=""><?php echo e(trans('admin.select')); ?></option>
								<option value="1"><?php echo e(trans('admin.new_customer')); ?></option>
								<option class="same_customer" value="2"><?php echo e(trans('admin.same_customer')); ?></option>
							</select>
						...
						<div class="form-group col-sm-6 to_customer_id" style="display:none">
							<label><?php echo e(trans('admin.select')); ?> <?php echo e(trans('admin.customer')); ?></label>
						...
						<div class="form-group col-sm-6 load_qty" style="display:none">
							<label><?php echo e(trans('admin.new')); ?> (M³)</label>
...
// body add class for sidebar end
	$(document).ready(function() {
		...
		$('body').on('click','.btn-del-note', function() {
			...
			$.ajax({
				url: "<?php echo e(Route('get_del_note_det','')); ?>/"+enc_id,
  				type:'POST',
  				data: {
		...
		$('#transfer_to').change(function(){
			...
			{
				$.ajax({
						url:"<?php echo e(Route('get_same_product_customer','')); ?>/"+enc_note_id,
						type:'GET',
						dataType:'json',
			...
			$.ajax({
				url: action,
  				...
  				success:function(response)
  				{
  					...
  					//var pdf_url = "<?php echo e(Route('dowload_del_note','')); ?>/"+response.id;
	...
	var excess_qty = "<?php echo e(trans('admin.excess_qty')); ?>";
...
<?php echo $__env->make('layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/cintvase/readymix.seeen.sa/resources/views/delivery/delivery_orders/index.blade.php ENDPATH**/ ?>