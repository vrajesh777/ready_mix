		...
		<div class="col text-right">
			<ul class="list-inline-item pl-0">
                <li class="list-inline-item">
                    <button class="add btn btn-gradient-primary font-weight-bold text-white todo-list-add-btn btn-rounded" id="add-task" data-toggle="modal" data-target="#add_item" onclick="form_reset()">{{ trans('admin.add') }} {{ trans('admin.parts') }}</button>
                </li>
            </ul>
		</div>
	</div>
					...
					<table class="table table-striped table-nowrap custom-table mb-0" id="itemsTable">
						<thead>
							<tr>
								...
								<th>{{ trans('admin.part') }} {{ trans('admin.name') }}</th>
	...
	<div class="modal-dialog" role="document">
		...
		<div class="modal-content">
			...
			<div class="modal-header">
                <h4 class="modal-title text-center top_title">{{ trans('admin.add') }} {{ trans('admin.parts') }}</h4>
                <button type="button" class="close xs-close" data-dismiss="modal">×</button>
            </div>

			<div class="modal-body">

				<form method="post" action="{{ Route('vc_part_store') }}" id="frmAddItem" enctype="multipart/form-data">
		            {{ csrf_field() }}
					<div class="row">
								...
								<div class="tab-pane show active">
									<div class="row">
										<div class="form-group col-sm-6">
											<label class="col-form-label">{{ trans('admin.part') }} {{ trans('admin.name') }}<span class="text-danger">*</span></label>
			            					<input type="text" class="form-control"  name="commodity_name" id="commodity_name" placeholder="{{ trans('admin.part') }} {{ trans('admin.name') }}" data-rule-required="true">
										</div>
									</div>
								</div>
							</div>
...
<script type="text/javascript">
	var createUrl = "{{ Route('vc_part_store') }}";
	var updateUrl = "{{ Route('vc_part_update','') }}";
	...
	{
		var title = "{{ trans('admin.edit') }}"+" {{ trans('admin.part') }}";
