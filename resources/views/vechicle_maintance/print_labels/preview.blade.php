		...
		@endphp
			...
			@if(!empty($print['name']))
				<span style="display: block !important">
					{{$details['details']->part->commodity_name ?? ''}}
				</span>
			@endif

			{{-- Variation --}}
			@if(!empty($print['variations']) && $details['details']->is_dummy != 1)
				{{$details['details']->part_no}}
			@endif

			{{-- Price --}}
			@if(!empty($print['price']))
				<b>Price:</b> {{ number_format($details['details']->buy_price,2)}}
			@endif

			<br>
			{{-- Barcode --}}
			<img class="center-block" style="max-width:90%; !important;height: {{$barcode_details->height*0.24}}in !important;" src="data:image/png;base64,{{DNS1D::getBarcodePNG($details['details']->part_no,'C128', 3,30,array(39, 48, 54), true)}}">
