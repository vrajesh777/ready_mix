...
class PumpHelpController extends Controller
{
	...
	public function __construct()
	{
		...
		$this->auth                 = auth();
        $this->arr_view_data        = [];
        $this->module_title         = trans('admin.pump_helper');
        $this->module_view_folder   = "pump_helper";
        $this->module_url_path      = url('/pump_helper');
	}
    ...
    public function index()
    {
        ...
        $obj_data = $this->BaseModel->whereHas('role', function(){})
                                    ...
                                    ->where('role_id', config('app.roles_id.pump_helper'))
                                    ->get();
    ...
    public function store(Request $request)
    {
        ...
        $role_id = config('app.roles_id.pump_helper');
        ...
        if($status)
        {
            ...
            $arr_resp['message']        = trans('admin.pump_helper').' '.trans('admin.added_successfully');
        }
        else
        {
            $arr_resp['status']         = 'error';
            $arr_resp['message']        = trans('admin.added_error').' '.trans('admin.pump_helper');
        }
        return response()->json($arr_resp, 200);
    }
