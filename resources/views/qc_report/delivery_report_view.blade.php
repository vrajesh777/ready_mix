...
{{ csrf_field() }}
<?php $is_cube = isset($arr_data['cube_details'][0])?$arr_data['cube_details'][0]['type'] == 'cube':false;?>
<input type="hidden" name="is_cube" id="is_cube" value="{{ $is_cube }}">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size:14px; font-family:'Arial', sans-serif;">
	...
	<tr>
		<td colspan="2" style="line-height:30px;border:1px solid #ccc;"><strong style="">&nbsp;&nbsp;{{ $is_cube ? trans('admin.compressive_strength_on_cube') : trans('admin.compressive_strength_on_cylinder') }}:&nbsp;&nbsp;</strong>{{ $arr_data['order_details']['order']['contract']['compressive_strength'] ?? '' }}&nbsp;&nbsp;</td>
...
<table class="table-input" width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size:14px; font-family:'Arial', sans-serif;">
	...
	<tr>
		...
		<td style="line-height:30px;border:1px solid #ccc;border-left:none">&nbsp;&nbsp;<strong style="">{{ $is_cube ? trans('admin.no_of_cubes_tested') : trans('admin.no_of_cylinder_tested') }}:</strong><input type="text" value="{{ $arr_data['no_of_cubes'] }}" name="no_of_cube_tested"  id="no_of_cube_tested" onchange="addCube(this.value)" class="form-control" placeholder="{{ $is_cube ? trans('admin.no_of_cubes_tested') : trans('admin.no_of_cylinder_tested') }}" data-rule-number="true"  data-rule-required="true">&nbsp;&nbsp;</td>
	</tr>
...
function calculate_density(currentRow)
{
     ...
     if(!isNaN(weight) && !isNaN(area) && !isNaN(height))
     {
     	...
     	*/
		 var apiUrl  = "{{ $module_url_path }}/calculate_density";
...
function calculate_avg_days()
{
	...
	var apiUrl  = "{{ $module_url_path }}/calculate_avg_days";
...
function getCalculatedDays()
{
	...
	var response = '';
	var apiUrl  = "{{ $module_url_path }}/calculate_tested_date";
