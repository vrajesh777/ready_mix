...
<div class="card">
	...
	<div class="card-header">
		<h4 class="card-title m-0">Record & Approve Payment</h4>
	</div>
	<div class="card-body">
		<div class="row">
		...
		<table class="table table-striped table-nowrap custom-table mb-0" id="shiftsTable">
			...
			<tbody>
				@if(isset($arr_master_salary) && sizeof($arr_master_salary)>0)
					@foreach($arr_master_salary as $salary)
						<tr>
	...
	<div class="modal-dialog" role="document">
		...
		<div class="modal-content">
			<div class="modal-header">
                <h4 class="mb-0">{{ trans('admin.approve_payroll') }}</h4>
                <button type="button" class="close xs-close" data-dismiss="modal">×</button>
            </div>

			<div class="modal-body">
				<form method="POST" action="{{ Route('record_payment') }}" name="frm_approve_payment" id="frm_approve_payment">
				{{ csrf_field() }}
					<input type="hidden" name="arr_emp_salary" value="{{ isset($arr_emp_salary)?json_encode($arr_emp_salary):'' }}">
					<input type="hidden" name="enc_id" value="{{ $enc_id ?? '' }}">
					<p>{{ trans('admin.approve_this_payroll_for') }} {{ $monthName ?? '' }}, {{ isset($arr_pay_run['for_year'])?date('Y',strtotime($arr_pay_run['for_year'])):'' }}. {{ trans('admin.once_it_approve_it') }} {{ isset($arr_pay_run['pay_date'])?date('d/m/Y',strtotime($arr_pay_run['pay_date'])):'' }}.</p>
					<div class="submit-section mt-0 text-left">
	                  	<button type="submit" class="border-0 btn btn-primary btn-gradient-primary btn-rounded">{{ trans('admin.record_payment') }}</button>&nbsp;&nbsp;
