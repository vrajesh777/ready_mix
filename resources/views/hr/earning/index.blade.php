		...
		<div class="col text-right">
			<ul class="list-inline-item pl-0">
                ...
                <li class="list-inline-item">
                    <button class="add btn btn-gradient-primary font-weight-bold text-white todo-list-add-btn btn-rounded" id="add-task" data-toggle="modal" data-target="#add_reimbs" onclick="form_reset()">{{ trans('admin.add_earning') }}</button>
                </li>
                {{-- @endif --}}
            </ul>
		</div>
					...
					<table class="table table-striped table-nowrap custom-table mb-0 datatable">
						<thead>
							<tr>
								...
								<th>{{ trans('admin.name_in_payslip') }}</th>
							...
							@if(isset($arr_data) && sizeof($arr_data)>0)
								@foreach($arr_data as $key => $value)
									<tr>
										...
										<td>{{ $value['cal_type'] ?? '' }}</td>
										{{-- <td>{{ $value['pay_type'] ?? '' }}
											@if($value['cal_type'] == 'percentage') ,{{ $value['cal_value'] }} % of {{ $value['pay_on'] ?? '' }} @else ,Flat amount @if($value['cal_type']!=''){{ $value['cal_value'] }} @endif @endif
										</td> --}}
										{{-- @if($obj_user->hasPermissionTo('pumps-update')) --}}
										<td>
											@if($value['is_active'] == '1')
												<a class="btn btn-success btn-sm" href="{{ Route('earning_deactivate', base64_encode($value['id'])) }}" onclick="confirm_action(this,event,'Do you really want to Deactivate this record ?');" title="Deactive">{{ trans('admin.active') }}</a>
											@else
												<a class="btn btn-danger btn-sm" href="{{ route('earning_activate',base64_encode($value['id'])) }}" onclick="confirm_action(this,event,'Do you really want to Activate this record ?');" title="Active">{{ trans('admin.deactive') }}</a>
											@endif
										</td>
	...
	<div class="modal-dialog" role="document">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="row">
			        <div class="col-md-12">
			            <form method="post" action="{{ Route('earning_store') }}" id="frmAddPump">
			            	{{ csrf_field() }}
			            	<input type="hidden" name="action" value="create">
			            	{{-- <input type="hidden" name="earning_type_id"> --}}
							<div class="form-group row">
								{{-- <div class="col-sm-6">
									<label class="col-form-label">Earning Type<span class="text-danger">*</span></label>
		                            <select class="form-control" name="earning_type_id" id="earning_type_id" data-rule-required="true">
		                            	<option value="">Select</option>
		                                @if(isset($arr_earning_type) && sizeof($arr_earning_type)>0)
		                                	@foreach($arr_earning_type as $type)
		                                		<option value="{{ $type['id'] ?? '' }}">{{ $type['name'] ?? '' }}</option>
		                                	@endforeach
		                                @endif
		                             </select>
		                             <label class="error" id="earning_type_id_error"></label>
								</div> --}}
								<div class="col-sm-6">
									<label class="col-form-label">{{ trans('admin.earning_name') }}<span class="text-danger">*</span></label>
								...
								<div class="col-sm-6">
									<label class="col-form-label">{{ trans('admin.name_in_payslip') }}<span class="text-danger">*</span></label>
...
<script type="text/javascript">
	...
	var createUrl = "{{ Route('earning_store') }}";
	var updateUrl = "{{ Route('earning_update','') }}";
	...
	var earning_op = "{{ trans('admin.select') }} {{ trans('admin.earning_op') }}";
	var earning_helper = "{{ trans('admin.select') }} {{ trans('admin.earning_helper') }}";
