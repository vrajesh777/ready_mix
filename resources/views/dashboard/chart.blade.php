<script type="text/javascript">
      ...
      function drawPumpPieChart() {
        var total_orders = "{{ $arr_pump_data['total_orders']??0 }}";
        var tot_delivered_qty = "{{ $arr_pump_data['tot_delivered_qty']??0 }}";
        var tot_remaing_qty = "{{ $arr_pump_data['tot_remaing_qty']??0 }}";
      ...
      function drawSalesPieChart() {
        ...
        var total_sales       = "{{ $arr_sales_data['total_sales']??0 }}";
        var invoice_amount    = "{{ $arr_sales_data['invoice_amount']??0 }}";
        var need_to_collect   = "{{ $arr_sales_data['need_to_collect']??0 }}";
        //console.log(total_orders+' '+tot_delivered_qty+' '+tot_remaing_qty);
        var data = google.visualization.arrayToDataTable([
          ['Sales', 'Amount'],
      ...
      function drawCustRejPumpPieChart() {
        ...
        var pump_tot_cust_rejected_qty  = "{{ $arr_rej_pump_data['glob_cust_rejected_qty']??0 }}";
        var pump_tot_int_rejected_qty   = "{{ $arr_rej_pump_data['glob_int_rejected_qty']??0 }}";
        //console.log(total_orders+' '+tot_delivered_qty+' '+tot_remaing_qty);
        var data = google.visualization.arrayToDataTable([
          ['Pump', 'Quantity'],
