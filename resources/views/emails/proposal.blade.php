...
$arr_cust = $arr_est_data['cust_details']??[];

$enc_id = base64_encode($arr_est_data['id']??'');

$details_link = Route('view_estimate', $enc_id);
                ...
                <tr>
                    <td bgcolor="#FFFFFF" style="border:1px solid #e5e5e5;">
                        <table style="margin-bottom: 0;" width="100%" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <table style="margin-bottom: 0;" width="100%" border="0" cellspacing="0" cellpadding="0">
                                        <tr>
                                            <td style="text-align:center;background-color:#8f4799;">
                                                <a href="{{ url('/') }}"><img src="{{ asset('/') }}images/logo1.png" class="logo-bg" alt="logo"/></a>
                                            </td>
                            ...
                            <tr><td style="color: rgb(51, 51, 51); text-align: center; font-size: 19px; line-height: 24px; padding-top: 3px;">Welcome To {{config('app.project.title')}}</td></tr>
                            <tr><td style="color: #333333;font-size: 15px;padding-top: 3px;text-align: center;">Estimation</td></tr>
                            <tr>
                                <td height="40"></td>
                            </tr>
                            <tr>
                                <td style="color: #333333; font-size: 16px; padding: 0 30px;">
                                    Dear <span style="color:#8f4799;font-family: 'Latomedium',sans-serif;">{{ $arr_cust['first_name'] ?? '' }} {{ $arr_cust['last_name'] ?? '' }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td style="color: #545454;font-size: 15px;padding: 12px 30px;">
                                    <p>Please find the attached estimate <strong>#{{ $arr_est_data['est_num']??'' }}</strong></p>
                                    <p><strong> Estimate status:</strong> {{ ucfirst($arr_est_data['status']??'') }}
                                    </p>
                                    <p>You can view the estimate on the following link:<br><a href="{{ $details_link??'' }}">{{ $details_link??'' }}</a></p>
                                    <p>We look forward to your communication.</p>
                                    <br>
                                    <p>Kind Regards,<br>{{config('app.project.title')}}</p>
                                </td>
                            </tr>
