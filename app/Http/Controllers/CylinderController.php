...
class CylinderController extends Controller
{
    public function __construct(DeliveryNoteModel $DeliveryNoteModel)
    {
        $this->auth                 = auth();
        $this->arr_view_data        = [];
        $this->module_title         = "Cylinder";
        $this->module_view_folder   = "cylinder";
        $this->module_url_path      = url('/cylinder');
        $this->DeliveryNoteModel    = $DeliveryNoteModel;
        $this->DelNoteQcCubeDetailModel    = new DelNoteQcCubeDetailModel();
    }
    public function index($report_type="new")
    {
    ...
    public function get_valid_date(Request $request)
    {
    	
    	$delivery_date       = $request->input('delivery_date','');
    	$current_date_tested = $request->input('current_date_tested','');
    	$modified_date       = $current_date_tested;
    	if($delivery_date!="")
    	{
    		$date_1        = date('Y-m-d', strtotime(str_replace('/', '-', $delivery_date)));
    		$date_2        = date('Y-m-d', strtotime(str_replace('/', '-', $current_date_tested)));
    		//$no_of_days    = (int)dateDifference($date_1,$date_2);
    		//$modified_date      = date('Y-m-d',strtotime($date_1."+".$no_of_days."days"));

    		$sum_dates     = strtotime($date_1) + strtotime($date_2);
    		$modified_date = date('Y-m-d',strtotime($sum_dates));
    	}
    	echo $modified_date;
    ...
    public function store_cube_details(Request $request)
    {
        ...
        $arr_delivery_note['cylinder_slump']       = $form_data['slump']??'';
        $arr_delivery_note['cylinder_no_of_cubes'] = $form_data['no_of_cube_tested']??'';
        $arr_delivery_note['cylinder_comp_method'] = $form_data['compaction']??'';
        $arr_delivery_note['cylinder_air_content'] = $form_data['air_content']??'';
        $arr_delivery_note['cylinder_sampled_by']  = $form_data['sampled_by']??'';
        $arr_delivery_note['cylinder_avg_at_days'] = $form_data['avg_at_days']??'';
        ...
        {
            Session::flash('success',trans('admin.cylinder').' '.trans('admin.details').' '.trans('admin.updated_successfully'));
        }
        else
        {
    ...
    public function delete_cube_details($cube_id,$delivery_no)
    {
        ...
        if($delete_status)
        {
            Session::flash('success','Cylinder details deleted successfully'); 
        }
        else
        {
